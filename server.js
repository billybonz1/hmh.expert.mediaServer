...
io.on('connection', function(socket) {
    ...
    function findUserSocketIDbyID(id, usersOnline){
        var result = 0;
        usersOnline.forEach(function(pages, i){
            var socketsArr = [];
            if(pages.length > 0){
                pages.forEach(function(user){
                    if(user.id == id){
                        socketsArr.push(user.socketid)
                    }
                });
    ...
    socket.on("userOnline", function (data) {
        ...
        var findedUserInfo = findUserSocketIDbyID(data.id, usersOnline);
        if(findedUserInfo != 0){
            usersOnline[findedUserInfo[1]].push(data);
        }else{
            usersOnline.push(Array(data));
        }
        this.broadcast.emit("userOnline", data);
        this.emit("usersOnline", usersOnline);
    });


    socket.on("userTyping", function (data) {
        var findedUserInfo = findUserSocketIDbyID(data.to_id, usersOnline);
        if(findedUserInfo != 0){
            findedUserInfo[0].forEach(function(socketid){
    ...
    socket.on("message", function (message) {
        if(message.to_id == 0){
            ...
            var findedUserInfo = findUserSocketIDbyID(message.to_id, usersOnline);
            if(findedUserInfo != 0){
                findedUserInfo[0].forEach(function(socketid){
    ...
    socket.on("readMessages", function (data) {
        var findedUserInfo = findUserSocketIDbyID(data.userID, usersOnline);
        if(findedUserInfo != 0){
            findedUserInfo[0].forEach(function(socketid){
                io.to(socketid).emit("readMessages", data);
            });
        }
    });


    socket.on("checkUserOnline", function (data) {
        var findedUserInfo = findUserSocketIDbyID(data.id, usersOnline);
        var result = 0;
        if(findedUserInfo != 0){
            result = 1;
        ...
        data.result = result;
        socket.emit('checkedUserOnline', data);
    });


    socket.on("userCall", function (data) {
        var findedUserInfo = findUserSocketIDbyID(data.id, usersOnline);
        if(findedUserInfo != 0){
            findedUserInfo[0].forEach(function(socketid){
                io.to(socketid).emit("userCall", data);
            });
        }
    });
    ...
    socket.on('disconnect', function () {
        ...
        var userIndex, index, userID;
        usersOnline.forEach(function(pages, i){
            if(pages !== null && pages.length > 0){
                pages.forEach(function(item, j){
                    if(item !== null){
        ...
        if(index >= 0 && userIndex >= 0){
            usersOnline[userIndex].splice(index, 1);
            if(usersOnline[userIndex].length == 0){
                usersOnline.splice(userIndex, 1);
                this.broadcast.emit('userDisconnected', userID); 
            }
        }
    });
